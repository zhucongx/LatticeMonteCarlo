...
std::map<std::string, double> ShortRangeOrder::FindWarrenCowley(const size_t shell_number) const
{
  ...
  for (const auto type : element_set_) {
    ...
    concentration[type] =
        static_cast<double>(element_list_map.at(type).size()) / static_cast<double>(config_.GetNumAtoms());
  }
  size_t num_bonds;
  switch (shell_number) {
    case 1:
      num_bonds = constants::kNumFirstNearestNeighbors;
      break;
    case 2:
      num_bonds = constants::kNumSecondNearestNeighbors;
      break;
    case 3:
      num_bonds = constants::kNumThirdNearestNeighbors;
      break;
    default:
  ...
  for (const auto &element1 : element_set_) {
    size_t num_all_bonds = element_list_map.at(element1).size() * num_bonds;
    std::map<Element, size_t> ct_this_pair_map{};
    for (const auto &element2 : element_set_) { ct_this_pair_map[element2] = 0; }
    for (const auto &atom_id1 : element_list_map.at(element1)) {
      std::vector<size_t> neighbor_list;
      switch (shell_number) {
        case 1:
          neighbor_list = config_.GetFirstNeighborsAtomIdVectorOfAtom(atom_id1);
          break;
        case 2:
          neighbor_list = config_.GetSecondNeighborsAtomIdVectorOfAtom(atom_id1);
          break;
        case 3:
          neighbor_list = config_.GetThirdNeighborsAtomIdVectorOfAtom(atom_id1);
          break;
        default:
          throw std::invalid_argument("Unknown shell number: " + std::to_string(shell_number));
      }
      for (const auto &atom_id2 : neighbor_list) {
        const auto &this_element = config_.GetElementAtAtomId(atom_id2);
        if (this_element == ElementName::X) { continue; }
        ct_this_pair_map[this_element]++;
      }
    }
    for (auto [element2, ct_this_pair] : ct_this_pair_map) {
      ...
      double pij = static_cast<double>(ct_this_pair) / static_cast<double>(num_all_bonds);
      warren_cowley[key] =
          (pij - concentration[element2]) / (static_cast<double>(element1 == element2) - concentration[element2]);
...
ShortRangeOrder::FindProbabilityCluster(size_t shell_number, const std::vector<size_t> &cluster_atom_id_list) const
{
  std::map<std::string, double> pij_map;
  for (const auto &element1 : element_set_) {
    for (const auto &element2 : element_set_) { pij_map[element1.GetString() + "-" + element2.GetString()] = 0; }
  }
  const auto element_list_map_all = config_.GetElementAtomIdVectorMap();
  std::map<Element, std::vector<size_t>> element_list_map{};
  for (const auto &element : element_set_) { element_list_map[element] = {}; }
  for (const auto &atom_id : cluster_atom_id_list) {
    auto element = config_.GetAtomVector()[atom_id].GetElement();
    if (element == ElementName::X) { continue; }
    element_list_map.at(element).push_back(atom_id);
  ...
  for (const auto &element1 : element_set_) {
    ...
    for (const auto &atom_id1 : element_list_map.at(element1)) {
      ...
      for (const auto &atom_id2 : neighbor_list) {
        ...
        if (element2 == ElementName::X) { continue; }
...
std::map<std::string, double> ShortRangeOrder::FindProbability(size_t shell_number) const
{
  ...
  for (const auto &element1 : element_set_) {
    const size_t num_all_bonds = element_list_map.at(element1).size() * num_bonds;
    std::map<Element, size_t> ct_this_pair_map{};
    for (const auto &element2 : element_set_) { ct_this_pair_map[element2] = 0; }
    for (const auto &atom_id1 : element_list_map.at(element1)) {
      std::vector<size_t> neighbor_list;
    ...
    for (auto [element2, ct_this_pair] : ct_this_pair_map) {
      std::string key = element1.GetString() + "-" + element2.GetString();
      const double pij = static_cast<double>(ct_this_pair) / static_cast<double>(num_all_bonds);
      pij_map[key] = pij;
    }
  }
  return pij_map;
