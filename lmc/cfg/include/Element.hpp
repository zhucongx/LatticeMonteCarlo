...
#include <map>
#include <string>
enum class ElementName { X, Al, Mg, Zn, Cu, Sn, pAl, pMg, pZn, pCu, pSn };
class Element {
 public:
  Element() = default;
  constexpr explicit Element(ElementName element_type)
      : element_name_(element_type) {}
  explicit Element(const std::string &element_string) {
    const std::map<std::string, ElementName> ElementStrings{{"X", ElementName::X},
                                                            {"Al", ElementName::Al},
                                                            {"Mg", ElementName::Mg},
                                                            {"Zn", ElementName::Zn},
                                                            {"Cu", ElementName::Cu},
  ...
  constexpr explicit operator ElementName() const {
  ...
  [[nodiscard]] std::string GetString() const {
    switch (element_name_) {
      ...
      default:
        throw std::invalid_argument("Unexpected pseudo element");
  ...
  [[nodiscard]] Element GetPseudo() const {
    switch (element_name_) {
      ...
      case ElementName::Sn:
        return Element(ElementName::pSn);
        // omit default case to trigger compiler warning for missing cases
